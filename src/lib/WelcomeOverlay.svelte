<script lang="ts">
  ...
  // → what's new; normal launch / downgrade → renders nothing). The what's-new
  // screen renders the scrollable per-version log (VersionLog.svelte).
  // Dismissal is one click (the button, the ×, the backdrop, or Escape via
  ...
  // Which surface the overlay shows: the post-update what's-new screen
  // renders the full version LOG (every recorded release, newest first,
  // scrollable — VersionLog.svelte) so highlights from previous updates stay
  // reachable. Only the first-install welcome keeps its own layout.
  const mode = $derived(firstLaunch.screen)
  const isLog = $derived(mode === 'whats-new')
...
{#if firstLaunch.visible}
  ...
  >
    ...
    {:else}
      <!-- Version log (post-update what's-new): every recorded release,
           newest first, scrollable — the running version leads with the
           accent header. -->
    ...
    <div class="welcome-actions">
      <button type="button" class="welcome-primary" onclick={dismiss}
        >{mode === 'welcome' ? t('welcome_getStarted') : t('whatsnew_continue')}</button
      >
    </div>
  </div>
{/if}
