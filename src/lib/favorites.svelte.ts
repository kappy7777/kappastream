...
export type LiveStatus =
  ...
  | {
      ...
      // tests — stay valid). collabViewers non-null = in a shared session
      // (combined viewership).
      collabViewers?: number | null
      collabOthers?: number
      collabAvatar?: string
      // Numeric Twitch user ID — the key the collaboration roster query
      // (channel(id:)) needs.
      userId?: string
  ...
  try {
    ...
    if (cs.live) {
      return {
        ...
        collabOthers: cs.collabOthers,
        collabAvatar: cs.collabAvatar,
...
/**
 * Stream Together / costream badge data for a LiveStatus: present when the
...
export function collabBadge(s: LiveStatus): { others: number; avatar: string } | null {
  ...
  if (s.collabViewers == null && !s.collabOthers) return null
  return { others: s.collabOthers ?? 0, avatar: s.collabAvatar ?? '' }
}
...
export class FavoritesStore {
    ...
    arr.sort((a, b) => {
      ...
      if (settings.sortMode === 'auto') {
        if (aLive === 0) {
          ...
          // Stream Together / costream (mirroring the number the sidebar
          // displays); own viewers otherwise.
          const av = a.status.state === 'live' ? (a.status.collabViewers ?? a.status.viewers) : 0
          const bv = b.status.state === 'live' ? (b.status.collabViewers ?? b.status.viewers) : 0
  ...
  private async pollOnce(): Promise<void> {
    ...
    try {
      ...
      await this.fetchSessionRosters(statuses, versions)
    } catch {
      if (this.disposed) return
  ...
  // The users(logins:) batch reports WHICH channels are in a Stream Together
  // / costream session (collabViewers) but not the full roster. This
  // follow-up fetches the real participant lists — logins, avatars, roles,
  // for EVERY member (favorited or not) — via the same anonymous GQL path
  ...
  private async fetchSessionRosters(
    ...
    try {
      const rosters = await fetchCollaborators(sessionIds)
      if (this.disposed) return
  ...
  private applyCollabRosters(
    ...
    if (this.disposed || rosters.size === 0) return
    let changed = false
    for (const cs of statuses) {
      if (!cs.login || !cs.userId) continue
      const members = rosters.get(cs.userId)
      if (!members) continue // not in a session (or no roster served)
      if (!this.has(cs.login)) continue // removed mid-flight
      if ((this.entryVersions.get(cs.login) ?? 0) !== (versions.get(cs.login) ?? 0)) {
        continue // a newer fetch owns this channel
      }
      const prev = this.statuses.get(cs.login)
