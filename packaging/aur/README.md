...
## Prerequisites (your Arch machine, not this container)
...
1. **Tag a release** so `pkgver()` can switch from `r<count>.<sha>` to
   `git describe`-based versioning (see the commented alternative in the PKGBUILD).
